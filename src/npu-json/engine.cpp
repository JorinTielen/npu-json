...
void Engine::handle_open_structure(StructureType structure_type) {
  ...
  // When FindIndex passes the container bracket to a handler that expects a
  // different structure (e.g. $[0].user → OpenObject receives '['), skip it.
  if (initial_structural_character != nullptr) {
    char c = json_c[*(structural_character)];
    if ((c == '[' && structure_type != StructureType::Array) ||
        (c == '{' && structure_type != StructureType::Object)) {
