...
#ifdef NPU_JSON_NPU_MATRIX_BACKEND
...
TEST_CASE("npu matrix kernel indexes one chunk correctly") {
  auto json = std::string(R"({"a":1,"b":"x, {[]}:","c":[true,{"d":"\\\"z"}]})");

  auto kernel = std::make_unique<matrix::npu::NPUMatrixKernel>(json);
  auto indexer = std::make_unique<matrix::npu::NPUMatrixIndexer>(*kernel, json);
...
TEST_CASE("npu matrix kernel handles chunk carries across boundaries") {
...
TEST_CASE("npu matrix backend matches expected counts on representative queries") {
...
TEST_CASE("npu matrix combined kernel matches CPU kernel on representative inputs") {
  auto json = std::string(R"({"a":1,"b":"x, {[]}:","c":[true,{"d":"\\\"z"}]})");

  auto npu_matrix_kernel = std::make_unique<matrix::npu::NPUMatrixKernel>(json);
  auto cpu_kernel = std::make_unique<npu::Kernel>(json);

  auto npu_matrix_index = std::make_unique<npu::ChunkIndex>();
  auto cpu_index = std::make_unique<npu::ChunkIndex>();

  npu_matrix_kernel->call(npu_matrix_index.get(), 0, []{});
  cpu_kernel->call(cpu_index.get(), 0, []{});

  auto npu_matrix_structurals = collect_chunk_structurals(*npu_matrix_index);
  auto cpu_structurals = collect_chunk_structurals(*cpu_index);

  REQUIRE(npu_matrix_structurals == cpu_structurals);
}

#else

TEST_CASE("npu matrix backend tests are skipped for non-NPU-matrix builds") {
  SUCCEED("Build without NPU_JSON_NPU_MATRIX_BACKEND");
}
